...
void Obs4ExtractingLightConeParticles(pmparticletype *pmparticles,int np,float nextastep,float maxd){
	int nx,ny,nz, nspace;
	...
	*/
	...
	OBSTEMWRITE(slcparticles,snlcp,myid,nid,4);
	/*
	for(i=0;i<snlcp;i++){
...
void Obs4SavingLightConeData(pmparticletype *pmparticles,int np,int nx,int ny,
	...
	OBSTEMREAD(slcparticles,snlcp,myid,nid,4);
	...
	*/
	...
	}
	{
		...
		sprintf(surveyfilename,"Olightcone4.main.%.5d",mstep);
		pp = lightcones;
		writedownlightconedata(surveyfilename,pp,nnp[0]);

		sprintf(surveyfilename,"Olightcone4.outer.%.5d",mstep);
		pp += nnp[0];
		writedownlightconedata(surveyfilename,pp,nnp[1]);

		sprintf(surveyfilename,"Olightcone4.inner.%.5d",mstep);
		pp += nnp[1];
