...
static slcparticletype *slcparticles[200];
static int snlcp[200];
...
void Obs0ExtractingLightConeParticles(pmparticletype *pmparticles,int np,float nextastep,float maxd,
		double SX0, double SY0, double SZ0, int mode){
	int nx,ny,nz, nspace;
	...
	*/
	...
	snlcp[mode] = 0;
	maxlcp = NPSTEP;
	slcparticles[mode] = (slcparticletype *)Malloc(sizeof(slcparticletype)*maxlcp,
			PPTR(slcparticles[mode]));
	for(j=0;j<1;j++){
		bp = pmparticles;
		for(i=0;i<np;i++){
			xp = XofP(bp) - SX0;
			...
			if(distsq >= rminsq && distsq < rmaxsq && distsq <51000000.L){
				dist = sqrt(distsq);
				slcparticles[mode][snlcp[mode]].x = xp;
				slcparticles[mode][snlcp[mode]].y = yp;
				slcparticles[mode][snlcp[mode]].z = zp;
				slcparticles[mode][snlcp[mode]].ovx = bp->vx;
				slcparticles[mode][snlcp[mode]].ovy = bp->vy;
				slcparticles[mode][snlcp[mode]].ovz = bp->vz;
				slcparticles[mode][snlcp[mode]].bp = bp;
				if(dist>=r1&& dist < r2) slcparticles[mode][snlcp[mode]].type = 0;
				else if(dist >=r2 && dist < r2+maxd) slcparticles[mode][snlcp[mode]].type = 1;
				else slcparticles[mode][snlcp[mode]].type = 2;
#ifdef INDEX
				slcparticles[mode][snlcp[mode]].indx = bp->indx;
#endif
				snlcp[mode] ++;
				if(snlcp[mode]>= maxlcp){
					maxlcp += NPSTEP;
					slcparticles[mode] = 
						(slcparticletype*)Realloc(slcparticles[mode],
	...
	slcparticles[mode] = (slcparticletype*)Realloc(slcparticles[mode],
		sizeof(slcparticletype)*snlcp[mode]);
	OBSTEMWRITE(slcparticles[mode],snlcp[mode],myid,nid,0);
...
void Obs0SavingLightConeData(pmparticletype *pmparticles,int np,int nx,int ny,
	...
	OBSTEMREAD(slcparticles[mode],snlcp[mode],myid,nid,0);

	MPI_Reduce(&snlcp[mode],&tsnlcp,1,MPI_INT,MPI_SUM,0,MPI_COMM_WORLD);
	...
	*/
	...
	nnp[0] = nnp[1] = nnp[2] =0;
	tp = slcparticles[mode];
	for(i=0;i<snlcp[mode];i++){
		bp = tp->bp;
		tp->nvx = bp->vx;
		tp->nvy = bp->vy;
		tp->nvz = bp->vz;
	...
	for(i=0;i<snlcp[mode];i++){
		nnp[slcparticles[mode][i].type]++;
	}
	if(snlcp[mode]>0) qsort(slcparticles[mode],snlcp[mode],sizeof(slcparticletype),ssorttype);
	if(snlcp[mode]>0) lightcones = (lightconetype *)Malloc(sizeof(lightconetype)*snlcp[mode],
			PPTR(lightcones));
	for(i=0;i<snlcp[mode];i++){
		lightcones[i].x = slcparticles[mode][i].x;
		lightcones[i].y = slcparticles[mode][i].y;
		lightcones[i].z = slcparticles[mode][i].z;
		lightcones[i].vx = slcparticles[mode][i].vx;
		lightcones[i].vy = slcparticles[mode][i].vy;
		lightcones[i].vz = slcparticles[mode][i].vz;
		lightcones[i].ovx = slcparticles[mode][i].ovx;
		lightcones[i].ovy = slcparticles[mode][i].ovy;
		lightcones[i].ovz = slcparticles[mode][i].ovz;
		lightcones[i].nvx = slcparticles[mode][i].nvx;
		lightcones[i].nvy = slcparticles[mode][i].nvy;
		lightcones[i].nvz = slcparticles[mode][i].nvz;
#ifdef INDEX
		lightcones[i].indx = slcparticles[mode][i].indx;
#endif
	}
	{
	...
	*/
	if(snlcp[mode]>0) Free(lightcones);
	Free(nnp);
	Free(slcparticles[mode]);
}
#undef npstep
