...
#define ObsVWHX(view)   ((view).E2.x)
#define ObsVWHY(view)   ((view).E2.y)
#define ObsVWHZ(view)   ((view).E2.z)

void ObsAnimate(pmparticletype *pmparticles,int np, Viewer *view, int nframe,
		int stepcount,int mode){
	float HOpticalDepth;
	int nx,ny,nz, nspace;
	...
	for(nowframe=0;nowframe<nframe;nowframe+=1){
		if(view[nowframe].nstep != stepcount) continue;
		...
		*/
		...
		vwvx = vwhy*vwz - vwhz*vwy;
		vwvy = vwhz*vwx - vwhx*vwz;
		vwvz = vwhx*vwy - vwhy*vwx;
		...
		dtheta = (Image.nx/2)/(PI/(2.+mode));
		{
			...
			for(i=0;i<np;i++){
				...
				for(j=0;j<nbox;j++){
					...
					mx =  (dtheta * theta)*cos(phi) + (Image.nx/2);
					my =  (dtheta * theta)*sin(phi) + (Image.ny/2);
					if(mx >=0 && mx < Image.nx && my >=0 && my < Image.ny) 
		...
		if(myid == 0){
			FILE *wp;
			MPI_Status status;
			sprintf(outfile,"%s.%.5d",view[nowframe].outfile,view[nowframe].frame);
			wp = fopen(outfile,"w");
