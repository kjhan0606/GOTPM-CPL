     !Simple test program to print out sigma_8 as a function of the CDM density
      ...
      program InitPower
        ...
        real matterpk(8192,3),rkarray(8192),Ts(8192)
...
!        print *,pamp, InData%MTrans%sigma_8(1,1)**2*boxsize**3
        ...
        call InitializePowers(CP%InitPower,CP%curv)
         ...
         nmatterpk = 8192
         call GetPower(minkh,dlnkh,matterpk,nmatterpk,Ts)
         do i = 1, nmatterpk
            rkarray(i) = minkh*exp((i-1)*dlnkh)
            rk = rkarray(i)
            write(*,*) rkarray(i), matterpk(i,1),matterpk(i,2),matterpk(i,3)
         enddo
...
!        close(1)
      ...
      subroutine GetPower(minkh,dlnkh,matterpk,nmatterpk,Ts)
        ...
        real matterpk(nmatterpk,3),Ts(nmatterpk)
...
!        print *, Mtrans%num_q_trans,MTrans%TransferData(Transfer_kh,Mtrans%num_q_trans,itf)
         ...
         in = 1
         call Transfer_GetMatterPower(Mtrans,matterpk(1,1),itf,in,minkh,dlnkh,npoints)
         call Transfer_GetSelectedMatterPower(Mtrans,matterpk(1,2),itf,in,minkh,dlnkh,npoints,Transfer_cdm)
         call Transfer_GetSelectedMatterPower(Mtrans,matterpk(1,3),itf,in,minkh,dlnkh,npoints,Transfer_b)
         h = CP%H0/100
         logmink = log(minkh)
         do i = 1, npoints
            k = exp(logmink+dlnkh*(i-1))*h
            Ts(i) = matterpk(i,1)/ScalarPower(k,in)/(k*pi*twopi*h**3)
         enddo
