...
long Dump2PeriodicPosExceptZ(long ix0,long iy0,long iz0,long ixm,
		long ixp, long iym, long iyp, long izm, long izp,int nx,int ny,int nz,int nspace,
		particle *pos){
	long ix,iy,iz;
	long ix2,iy2,iz2;
	long mcell;
	...
	for(iz=iz0+izm;iz<iz0+izp;iz++){
		...
		for(iy=iy0+iym;iy<iy0+iyp;iy++){
			...
			for(ix=ix0+ixm;ix<ix0+ixp;ix++){
				...
				while(p){
					pos[npos].x = XofP(p) + lpx;
					pos[npos].y = YofP(p) + lpy;
					pos[npos].z = ZofP(p);
					pos[npos].bp = p;
					npos ++;
					p = p->next;
...
void PartialUpdateAccelUsingForceByParticles(float vfact2,long npos,particle *pos,float mass){
	...
	for(i=0;i<npos;i++){
		bp = pos[i].bp;
		bp->vx += vfact2*pos[i].ax*mass;
		bp->vy += vfact2*pos[i].ay*mass;
		bp->vz += vfact2*pos[i].az*mass;
	}
}
/*
...
float treecorrection(treeparticletype *particles,long np){
	...
	*/
	...
	if(0){
		...
		xmin = ymin = zmin = 1.e16;
		xmax = ymax = zmax =-1.e16;
		for(i=0;i<np;i++){
			xmin = min(xmin,XofP(particles+i));
			ymin = min(ymin,YofP(particles+i));
			zmin = min(zmin,ZofP(particles+i));
			xmax = max(xmax,XofP(particles+i));
			ymax = max(ymax,YofP(particles+i));
			zmax = max(zmax,ZofP(particles+i));
		}
		printf("P%d has %g %20.12g %g %20.12g %g %20.12g\n",myid,xmin,xmax,ymin,ymax,zmin,zmax);
...
#endif
		{
			...
			for(iy0=0;iy0<my;iy0+=nstride){
				...
				for(ix0=0;ix0<mx;ix0+=nstride){
					...
					/* Making tree buffer */
					ngrv = Dump2Grv(ix0,iy0,iz0,0,nstride,0,nstride,0,nstride,threadgrv,nx,ny,nz,nspace);
					if(ngrv > 0){
						npos = Dump2PeriodicPosExceptZ(ix0,iy0,iz0,-1,jxp,-1,jyp,-1,jzp,nx,ny,nz,nspace,
								threadpos);
						if(ngrv >= DIRECTSUM){
							box.x = ix0*CellWidth;
							box.y = iy0*CellWidth;
...
void Ghost(long iflag,float zstart,float zheight,float vfact2,
	...
	for(iy0=0;iy0<my;iy0+=nstride){
		...
		if(tnneighbor > 1){
			if(myid==0) fprintf(stdout,"Now entering into too thin slab: %d\n",tnneighbor);
...
#endif
		{
			...
			for(ix0=0;ix0<mx;ix0+=nstride){
				...
				if(ngrv > 0){
					...
					npos = Dump2PeriodicPosExceptZ(ix0,iy0,iz0pos,-1,jxp,-1,jyp,jzm,jzp,nx,ny,nz,nspace,
							threadpos);
					if(npos > 0){
						if(ngrv >= DIRECTSUM){
