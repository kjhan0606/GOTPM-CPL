...
void tree2pmtype(long nplong){
	 ...
	 * treeparticles and pmparticles point to same memory but are different types.
	 * Field-by-field copy through different-type pointers is undefined behavior
	 * at -O2, causing the compiler to reorder reads/writes and corrupt data. */
	for(i=0;i<nplong;i++){
		memcpy((char*)pmparticles + i*sizeof(pmparticletype),
		       (char*)treeparticles + i*sizeof(treeparticletype),
...
#endif
*/
int Zeldovich(int argc, char *argv[]){
	int i,j,k;
...
/* THIS IS FOR DEFINITION OF THE OPENMP */
	...
	if(0){
        ...
        set3dFftwInfo(nx, ny, nz, MPI_COMM_WORLD , &local_nz, &local_z_start);
	...
	if(icont ==0){
		...
		float *ffx,*ffy,*ffz;
		{
			...
			pmseedforce_(&np,pmparticles,&nsizepmparticletype,&nxpos,den,
					&simpar.omep,&simpar.omeplam,&simpar.wlam0, &simpar.wlam1, 
					&simpar.CsDE, &simpar.ken,
...
/* Ok now output to stdout to verify all is well */
	if( myid ==0 ) {
		...
		fprintf(stdout,"npow = %g Omega = %g OmegaBaryon = %g Lambda = %g\nBias_8Mpc = %g Rsmooth = %g\n\n", 
				(simpar.npow),simpar.omep,simpar.omepb,simpar.omeplam,simpar.bias8,simpar.rsmooth);
		fprintf(stdout,"\n*** Mesh Parameters ***\n");
...
void getpowerparams_(int *, float *, float*, float *, float *, float *, float *, float *,
		float *, float *, int *, float *, int *, char *, char *, int *);
			 
void getpowerparams_(nspace,size,amax,astep,npow,smooth,h,rth,
		bias8, As,iseed, omepb,powflag,powfile,inpapkfile, pkrescaling)
int *nspace;
float *size, *amax, *astep, *As;
float *npow;
int *iseed;
float *smooth, *h, *rth, *bias8;
float *omepb;
char *powfile,*inpapkfile;
int *powflag, *pkrescaling;
{
	int i;		 
	 ...
	 *pkrescaling = simpar.Pk_rescaling;
	 for(i=0;i<190;i++) powfile[i] = '\0';
	 if(simpar.powreadflag ==1) sprintf(powfile,"%s",simpar.powfilename);
	 if(simpar.powreadflag ==2) sprintf(inpapkfile,"%s",simpar.inpapkfilename);

//	 printf("power files : %s %s\n", powfile, simpar.powfilename);
}
