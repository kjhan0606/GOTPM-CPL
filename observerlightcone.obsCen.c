...
void CObs0ExtractingLightConeParticles(pmparticletype *pmparticles,
	...
	*/
	...
	OBSTEMWRITE(slcparticles,snlcp,myid,nid,8);
	/*
	for(i=0;i<snlcp;i++){
...
void CObs0SavingLightConeData(pmparticletype *pmparticles,int np,int nx,int ny,
	...
	OBSTEMREAD(slcparticles,snlcp,myid,nid,8);
	...
	*/
	...
	}
	{
		...
		sprintf(surveyfilename,"COlightcone.main.%.5d",mstep);
		pp = lightcones;
		writedownlightconedata(surveyfilename,pp,nnp[0]);

		sprintf(surveyfilename,"COlightcone.outer.%.5d",mstep);
		pp += nnp[0];
		writedownlightconedata(surveyfilename,pp,nnp[1]);

		sprintf(surveyfilename,"COlightcone.inner.%.5d",mstep);
		pp += nnp[1];
