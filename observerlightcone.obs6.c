...
void Obs6ExtractingLightConeParticles(pmparticletype *pmparticles,int np,float nextastep,float maxd){
	int nx,ny,nz, nspace;
	...
	*/
	...
	OBSTEMWRITE(slcparticles,snlcp,myid,nid,6);
	/*
	for(i=0;i<snlcp;i++){
...
void Obs6SavingLightConeData(pmparticletype *pmparticles,int np,int nx,int ny,
	...
	OBSTEMREAD(slcparticles,snlcp,myid,nid,6);
	...
	*/
	...
	}
	{
		...
		sprintf(surveyfilename,"Olightcone6.main.%.5d",mstep);
		pp = lightcones;
		writedownlightconedata(surveyfilename,pp,nnp[0]);

		sprintf(surveyfilename,"Olightcone6.outer.%.5d",mstep);
		pp += nnp[0];
		writedownlightconedata(surveyfilename,pp,nnp[1]);

		sprintf(surveyfilename,"Olightcone6.inner.%.5d",mstep);
		pp += nnp[1];
