...
void InitNslices(int nowzstart,int nowzfinal,int afterzstart,int afterzfinal){
	int i,j,k;
	int displ, recvsize, rc;
	MPI_Status mpistatus;
    recvsize = sizeof(int)*nid*nz;
    rc = MPI_Alloc_mem(recvsize, MPI_INFO_NULL, &nowslices);
    if (rc) {
      printf("Can't allocate memory in InitNslices\n");fflush(stdout);
      MPI_Abort(MPI_COMM_WORLD, 1);
    }
    rc = MPI_Alloc_mem(recvsize, MPI_INFO_NULL, &afterslices);
    if (rc) {
      printf("Can't allocate memory in InitNslices\n");fflush(stdout);
      MPI_Abort(MPI_COMM_WORLD, 1);
    }
    ...
    MPI_Win_create(nowslices,recvsize,sizeof(int),MPI_INFO_NULL,MPI_COMM_WORLD,&win);
	//if (myid != 0) {
      displ = myid*nz;
	  for (i=0;i<nid;i++) {
		if (i==myid) continue;
        MPI_Win_lock(MPI_LOCK_SHARED, i, 0, win);
        MPI_Put(&nowslices(myid,0), nz, MPI_INT, i, displ, nz, MPI_INT, win);
	...
	*/
	...
	/*if(myid ==0){
		MPI_Status rstatus;
		for(i=1;i<nid;i++){
	        printf("P%d before mpi_probe ",nz);
			MPI_Probe(MPI_ANY_SOURCE,0,MPI_COMM_WORLD,&rstatus);
	        printf("after mpi_probe & before mpi_Recv from %d ", rstatus.MPI_SOURCE);
			MPI_Recv(&afterslices(rstatus.MPI_SOURCE,0),nz,MPI_INT,rstatus.MPI_SOURCE,0,MPI_COMM_WORLD,&mpistatus);
	        printf("after mpi_recv\n ");
		}
	}
	else MPI_Send(&afterslices(myid,0),nz,MPI_INT,0,0,MPI_COMM_WORLD);*/
    MPI_Win_create(afterslices,recvsize,sizeof(int),MPI_INFO_NULL,MPI_COMM_WORLD,&win);
	//if (myid != 0) {
      displ = myid*nz;
	  for (i=0;i<nid;i++) {
		if (i==myid) continue;
        MPI_Win_lock(MPI_LOCK_SHARED, i, 0, win);
        MPI_Put(&afterslices(myid,0), nz, MPI_INT, i, displ, nz, MPI_INT, win);
        MPI_Win_unlock(i, win);
	  }
	//}
	MPI_Barrier(MPI_COMM_WORLD);
...
//	Free(nowslices);
   MPI_Win_free(&win);
   MPI_Free_mem(afterslices);
   MPI_Free_mem(nowslices);
}
...
void CheckAndCopyStride(float *nowslice,float *afterslice,
	...
	if (myid==0) printf("before mpi_reduce ");
	(void) MPI_Reduce(nstridenum,tmpnstridenum,nid,MPI_INT,MPI_SUM,
					  0,MPI_COMM_WORLD);
	if (myid==0) printf("after mpi_reduce at CheckAndCopyStride\n");
	if(myid==0) {
		for(i=0;i<nid;i++) {
	...
	if (myid==0) printf("after broadcasting at CheckAndCopyStride\n");
}
