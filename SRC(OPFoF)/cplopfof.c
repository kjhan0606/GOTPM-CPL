...
POSTYPE lx,ly,lz;
	...
	float size,hubble,npower,omep,omepb,omeplam,bias,smooth,wlam0, wlam1;
	int nx,ny,nz,nspace;
...
int main(int argc,char *argv[]){
	...
	if(argc != 3) {
		...
		*/
		...
		if(myid==0){
			printf("P%d size = %g hubble = %g wlambda1/2= %g/%g\n",myid,size,hubble, wlam0, wlam1);
	...
	box.x = box.y = box.z = 0.;
	box.width = nx;
	sprintf(halofile,"cplFoF_halo_cat.%.5d",nstep);
	sprintf(memparticlefile,"cplFoF_member_particle.%.5d",nstep);
	if(myid==0){
		...
		fwrite(&wlam0,sizeof(float),1,hfp);
		fwrite(&wlam1,sizeof(float),1,hfp);
