...
#define CellWidth 8.L /* The cellwidth is preferred to be same as rsphere */
#define invCellWidth 0.125L /* This is 1/CellWidth . */
#define rsphere 4.
#define nstride 1
float maintreetime;
long mx,my,mz;
...
long Dump2FoFPtl(long ix0,long iy0,long iz0,long ixm,long ixp,
	...
	point[npoint-1].sibling = NULL;
	return npoint;
}
int CountHaloCandidates(int nhalo,HaloBound *halo,
...
int Dump2Disk(treeparticletype *particles, long np){
	long i,j,k,nsave;
	...
	sprintf(outfile,"PreFoF.%.5d%.5d",simpar.stepcount,myid);
	wp = fopen(outfile,"w");
	write_head(wp,simpar);

	simpar.np = nporg; /* back to original np */
	/*
	fwrite(&(simpar.zmin),sizeof(float),1,wp);
	fwrite(&(simpar.zmax),sizeof(float),1,wp);
	*/
	...
	for(i=0;i<np;i++){
		if(particles[i].indx <= 0L){
			particles[i].indx = -1L*(particles[i].indx);
...
#undef NPSAVE
/*
int Dump2Disk(int nhalo, HaloBound *halo, treeparticletype *pdump,long npdump, 
		INT8 *localnpdump, int threadID, int nthread,INT8 nmaxdump,
		long maxnpincell,int myid, int nid){
	long mh,maxbuffer;
	int WGroupSize,i;
	FoFTPtlStruct *tmp;
	long localnp,nshift,now,npnow;
	int src,tgt;
	int iget,isend,itag=1;
	MPI_Status status;
#define GROUPID(a,b) ((a)/(b))

	maxbuffer =  2L*maxnpincell*nthread;
	nshift = 0;
	for(i=0;i<threadID;i++) nshift += localnpdump[i];
	now = npdump+nshift;
	...
	npnow = npdump;
	for(i=0;i<threadID;i++) npnow += localnpdump[i];

	if(threadID==0){
		WGroupSize = WGROUPSIZE;
		src = myid-1;
		tgt = myid+1;
		if((myid%WGroupSize) != 0 ) MPI_Recv(&iget,1,MPI_INT,src,itag,MPI_COMM_WORLD,&status);
		if((nmaxdump-npdump) <= maxbuffer){
			char outfile[190];
			FILE *wp;
			sprintf(outfile,"PreFoF.%.5d.%.5d",simpar.stepcount,myid);
			wp = fopen(outfile,"w");
			fwrite(pdump,sizeof(treeparticletype),npnow,wp);
			fclose(wp);
			npnow = 0;
		}
		if(GROUPID(myid,WGroupSize) == GROUPID(tgt,WGroupSize) && tgt < nid) 
...
#ifdef _OPENMP
#pragma omp parallel private(iy0,jyp,jxp,ix0,npoint,i,box)
#endif
		{
			int siy0;
			...
			for(iy0=0;iy0<my;iy0+=nstride){
				...
				for(ix0=0;ix0<mx;ix0+=nstride){
					...
					localnpdump[threadID] = 
						CountHaloCandidates(nhalo,threadhalo,threadpoint,npoint,
								fof_link, box);
					/*
					npdump = Dump2Disk(nhalo,threadhalo, pdump,npdump,localnpdump,threadID,
							nthreads, nmaxdump,maxnpincell,myid,nid);
							*/
				}
			}
		}
