...
void WriteIsolatedHalo(size_t nhalo, HaloBound *halobound,FoFTPtlStruct *ptl,
	...
	if(myid==0){
		printf("P0 is now saving isolated halo data\n");
		...
		for(i=1;i<nid;i++){
			...
			int jtag = 3098;
			MPI_Status status;
			MPI_Recv(&haloq,sizeof(HaloQ),MPI_BYTE,i,itag,MPI_COMM_WORLD,&status);
			while(haloq.np > 0) {
				MPI_Recv(linked,haloq.np*sizeof(particle),MPI_BYTE,i,jtag,MPI_COMM_WORLD,&status);
				fwrite(&haloq,sizeof(HaloQ),1,hfp);
				if((mp=fwrite(linked,sizeof(particle),haloq.np,pfp)) != haloq.np){
					fprintf(stderr,"Error writing data to disk %ld : %ld\n",mp,haloq.np);
				}
				MPI_Recv(&haloq,sizeof(HaloQ),MPI_BYTE,i,itag,MPI_COMM_WORLD,&status);
			}
		}
		...
		int itag = 1098;
		int jtag = 3098;
		printf("P%d is sending halo data of %d\n",myid,nhalo);
		for(j=0;j<nhalo;j++){
			if(halobound[j].boundflag ==0){
				ptr = halobound[j].sibling;
				np = 0;
				...
				if(np >= MinNumMem){
					haloq=haloproperty(linked,np);
					MPI_Send(&haloq,sizeof(HaloQ),MPI_BYTE,0,itag,MPI_COMM_WORLD);
					MPI_Send(linked,sizeof(particle)*haloq.np,MPI_BYTE,0,jtag,MPI_COMM_WORLD);
...
void WriteFinalHalo(size_t nhalo, HaloBound *halobound,FoFTPtlStruct *ptl,
	...
	for(j=0;j<nhalo;j++){
		ptr = halobound[j].sibling;
		while(ptr) ptr=ptr->sibling;
	}
	printf("P%d passed the test for nhalo= %d\n",myid,nhalo);
	MPI_Barrier(MPI_COMM_WORLD);
#endif
	if(myid==0){
		printf("P0 is now saving final halo data\n");
		hfp = fopen(halofile,"a");
		pfp = fopen(memparticlefile,"a");
		for(j=0;j<nhalo;j++){
			{
		...
		for(i=1;i<nid;i++){
			...
			printf("P0 has written member data from P%d\n",i);
		}
		fclose(hfp);fclose(pfp);
	}
	else{
		...
		for(j=0;j<nhalo;j++){
			{
				...
				if(np >= MinNumMem){
					haloq=haloproperty(linked,np);
					MPI_Send(&haloq,sizeof(HaloQ),MPI_BYTE,0,itag,MPI_COMM_WORLD);
					MPI_Send(linked,haloq.np*sizeof(particle),MPI_BYTE,0,jtag,MPI_COMM_WORLD);
