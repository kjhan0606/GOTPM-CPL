...
void Obs3ExtractingLightConeParticles(pmparticletype *pmparticles,int np,float nextastep,float maxd){
	int nx,ny,nz, nspace;
	...
	*/
	...
	OBSTEMWRITE(slcparticles,snlcp,myid,nid,3);
	/*
	for(i=0;i<snlcp;i++){
...
void Obs3SavingLightConeData(pmparticletype *pmparticles,int np,int nx,int ny,
	...
	OBSTEMREAD(slcparticles,snlcp,myid,nid,3);
	...
	*/
	...
	}
	{
		...
		sprintf(surveyfilename,"Olightcone3.main.%.5d",mstep);
		pp = lightcones;
		writedownlightconedata(surveyfilename,pp,nnp[0]);

		sprintf(surveyfilename,"Olightcone3.outer.%.5d",mstep);
		pp += nnp[0];
		writedownlightconedata(surveyfilename,pp,nnp[1]);

		sprintf(surveyfilename,"Olightcone3.inner.%.5d",mstep);
		pp += nnp[1];
