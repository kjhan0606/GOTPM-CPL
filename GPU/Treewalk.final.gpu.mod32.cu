...
// GPU Tree is working but GPU direct n-body does not work properly. 03/03/2008
#include<stdio.h>
#include<stdlib.h>
#include<math.h>
#include<stddef.h>
...
static int mydevid;
#define GetBlockSize1D(npos,threadSize) (((int)(ceilf((float)npos/(float)threadSize))))
...
static texture<float,1,cudaReadModeNormalizedFloat> tex1;
static texture<float4,1,cudaReadModeNormalizedFloat> tex4;
static cudaArray  *cuArray1, *cuArray4;
float *force1;
float4 *hforce4;


static texture<int2  ,1,cudaReadModeElementType> tex_cellsibdau;
static texture<int   ,1,cudaReadModeElementType> tex_ptclsib;
static texture<float4,1,cudaReadModeElementType> tex_cellbasic;
static texture<float4,1,cudaReadModeElementType> tex_cellquad1;
static texture<float4,1,cudaReadModeElementType> tex_cellquad2;
static texture<float4,1,cudaReadModeElementType> tex_ptclbasic;

static texture<int2  ,1,cudaReadModeElementType> texcu_cellsibdau;
static texture<int   ,1,cudaReadModeElementType> texcu_ptclsib;
static texture<float4,1,cudaReadModeElementType> texcu_cellbasic;
static texture<float4,1,cudaReadModeElementType> texcu_cellquad1;
static texture<float4,1,cudaReadModeElementType> texcu_cellquad2;
static texture<float4,1,cudaReadModeElementType> texcu_ptclbasic;
	...
	int ndev;
	{
		...
		if(mydevid >= ndev){
			...
			if(myid==0) fprintf(stderr,
					"###########################################################\n");
			fprintf(stderr,
					"P%d has now been initializing the CPU setting and textures\n",
					myid);
			fprintf(stderr,
					"P%d has gpu id=%d with shared global memsize= %d\n",
					myid,mydevid,mydev.totalGlobalMem);
			fprintf(stderr,
					"P%d has gpu Rev. %d.%d Clock %d khz\n",
		...
		*/
		...
		CU_SAFE_CALL( cudaMallocArray(&cuArray1,&tex1.channelDesc,NTextureSize,1));
		CU_SAFE_CALL( cudaMallocArray(&cuArray4,&tex4.channelDesc,NTextureSize4,1));

		CU_SAFE_CALL( cudaBindTextureToArray(tex1,cuArray1));
		CU_SAFE_CALL( cudaBindTextureToArray(tex4,cuArray4));


		CU_SAFE_CALL( cudaMemcpyToArray(cuArray1,0,0,hforce1,
					sizeof(float)*NTextureSize,cudaMemcpyHostToDevice));
		CU_SAFE_CALL( cudaMemcpyToArray(cuArray4,0,0,hforce4,
					sizeof(float4)*NTextureSize4,cudaMemcpyHostToDevice));

		if(myid==0) fprintf(stderr,"Now P1 initialized textures\n");
...
__device__ acceltype P2PGF(float4 urpmass, float4 mypos, acceltype acc){
	float3 r;
	r.x = mypos.x - urpmass.x;
	r.y = mypos.y - urpmass.y;
	r.z = mypos.z - urpmass.z;
	float dist2 = r.x*r.x+r.y*r.y+r.z*r.z;
	float rr = (0.5f*__log10f(dist2)-LOG10RMIN)* invLOG10RMAXmLOG10RMIN;
	if(rr>0. && rr < RSPHERERR) {
		float fplmf = urpmass.w * tex1D(tex1,rr);
		acc.x += r.x*fplmf;
		acc.y += r.y*fplmf;
		acc.z += r.z*fplmf;
	}
...
__device__ acceltype tile_calculation(float4 myPos, acceltype myAcc){
	extern __shared__ float4 sharedPos[];
#pragma unroll 8
	for(int i=0;i<blockDim.x; i++) {
		myAcc = P2PGF(sharedPos[i],myPos,myAcc);
	}
	return myAcc;
}
__global__ void Get_Gravity(int ngrv, float4 *grv, int npos, float4 *Pos, acceltype *acc){
	...
	int gjd;
	int ibstart,ibfinal;
	int totalblock = (int)ceilf((float)ngrv/(float)blockDim.x);
	{
		acceltype myAcc = {0.f,0.f,0.f,0.f};
		float4 myPos;
//		if(gid< npos) myPos = Pos[gid];
//		else myPos = make_float4(0.f,0.f,0.f,0.f);
		myPos = Pos[gid];
		ibstart = blockIdx.x;
		ibfinal = ibstart + totalblock;
		for(int i = ibstart;i<ibfinal;i++){
			gjd = (i%totalblock)*blockDim.x+threadIdx.x;
			sharedPos[threadIdx.x] = grv[gjd];
			__syncthreads();
			myAcc = tile_calculation(myPos,myAcc);
			__syncthreads();
		}
...
float4 *position, *nbody;
acceltype *accel;
static float4 *deposition, *degrv;
static acceltype *deaccel;
static int nposbuffer=NPosBuffer,nbodybuffer=NBodyBuffer;
static int dn_initialflag = 1;
void DIRECTINTIALIZE(){
	cudaMalloc(PPTR(deposition),sizeof(float4)*nposbuffer);
	cudaMalloc(PPTR(deaccel),sizeof(float4)*nposbuffer);
	cudaMalloc(PPTR(degrv),sizeof(float4)*nbodybuffer);
	dn_initialflag = 0;
}
...
void Direct_Nbody( particle *pos,int npos, TPtlStruct *grv,int ngrv,
	...
	int threadsize=64;// The proper value of threadsize has not been known until now
	                  // but it is supposed to be small due to the small number of grv.
	int nblocks = (int)ceil((double)npos/(double)threadsize);
	mpos = threadsize * nblocks;
	mgrv = threadsize *(int)ceil((double)ngrv/(double)threadsize);
	...
	if(mpos> nposbuffer){
		...
		cudaFree(deposition);
		cudaFree(deaccel);
		cudaMalloc(PPTR(deposition),sizeof(float4)*mpos);
		cudaMalloc(PPTR(deaccel),sizeof(acceltype)*mpos);
	}
	if(mgrv > nbodybuffer){
		nbodybuffer = mgrv;
		cudaFree(degrv);
		cudaMalloc(PPTR(degrv),sizeof(float4)*mgrv);
	}
	...
	// Here we use "mgrv" instead of "ngrv" because we want to reduce the number of
	// divergences in the kernel.
	for(int i=ngrv;i<mgrv;i++) nbody[i].w = 0.f;
	CUDA_SAFE_CALL(cudaMemcpy(degrv,nbody,sizeof(float4)*mgrv,cudaMemcpyHostToDevice));

	CUDA_SAFE_CALL(cudaMemcpy(deposition,position,sizeof(float4)*npos,cudaMemcpyHostToDevice));

//	Assign the maximum size of shared memory to this kernel.
	int sharedmemsize = threadsize * sizeof(float4);
	Get_Gravity<<<nblocks,threadsize,sharedmemsize>>>(ngrv,degrv,npos,deposition,deaccel);

	accel = (acceltype*)Malloc(sizeof(acceltype)*npos,PPTR(accel));
	CUDA_SAFE_CALL(cudaMemcpy(accel,deaccel,sizeof(acceltype)*npos,cudaMemcpyDeviceToHost));
