...
#endif
void Ghost(long,float,float,float,int,int,int,float);
...
void ParticleCellLinkedList(treeparticletype *particles, long np, float zstart){
	...
	/* si should be 4-byte integer since the OpenMP do not allow long integer 
				for running index of "for" */
	/*
#ifdef _OPENMP 
	int si;
#endif
...
*/
	...
	tp = particles;
	for(i=0;i<np;i++){
		ix = (long)(tp->x*invCellWidth);
		iy = (long)(tp->y*invCellWidth);
		iz = (long)((tp->z-zstart)*invCellWidth);
...
long Dump2PeriodicPosExceptZ(long ix0,long iy0,long iz0,long ixm,
		long ixp, long iym, long iyp, long izm, long izp,int nx,int ny,
		particle *pos){
	long ix,iy,iz;
	long ix2,iy2,iz2;
	long mcell;
...
#pragma omp parallel for 
	for(si=0;si<(int)np;si++){
		(pos[si].bp)->vx += vfact2*pos[si].ax;
		(pos[si].bp)->vy += vfact2*pos[si].ay;
		(pos[si].bp)->vz += vfact2*pos[si].az;
	}
#else
	for(i=0;i<np;i++){
		(pos[i].bp)->vx += vfact2*pos[i].ax;
		(pos[i].bp)->vy += vfact2*pos[i].ay;
		(pos[i].bp)->vz += vfact2*pos[i].az;
	}
#endif
}
extern float diff[NSPLINE][3],slope[NSPLINE][3],ran2nran,invran2nran;
/*
void DirectSummation(long ngrv,TPtlStruct *grv,long npos,particle *pos){
*/
inline void DirectSummation(long ngrv,TPtlStruct *grv,long npos,particle *pos){
	float tmpx,tmpy,tmpz,fplmf;
	float dist2,ptlmass,dist;
	long i,j,ntmp;
	float lgdist,rr,dr;
	for(i=0;i<npos;i++){
		pos[i].ax = pos[i].ay = pos[i].az = 0.;
		for(j=0;j<ngrv;j++){
			tmpx = pos[i].x - grv[j].r[0];
			tmpy = pos[i].y - grv[j].r[1];
			tmpz = pos[i].z - grv[j].r[2];
			dist2 = tmpx*tmpx + tmpy*tmpy + tmpz*tmpz;
			lgdist = 0.5*log10f(dist2);
			rr = (lgdist-LOG10RMIN)* invLOG10RMAXmLOG10RMIN;
			if(rr>0. && rr<RSPHERERR){
				ntmp = rr*(float)NSPLINE;
				dr =lgdist - ((float)ntmp*ran2nran+LOG10RMIN);
				fplmf = diff[ntmp][0] + slope[ntmp][0] * dr;
				pos[i].ax += tmpx* fplmf;
				pos[i].ay += tmpy* fplmf;
				pos[i].az += tmpz* fplmf;
			}
...
void PartialUpdateAccelUsingForceByParticles(float vfact2,long npos,particle *pos){
	long i;
	treeparticletype *bp;
	...
	for(i=0;i<npos;i++){
		bp = pos[i].bp;
		bp->vx += vfact2*pos[i].ax;
		bp->vy += vfact2*pos[i].ay;
		bp->vz += vfact2*pos[i].az;
	}
}
/*
...
long npmaxcell;
float treecorrection(treeparticletype *particles,long np, int nx, int ny, 
		int nz, float zstart, float zheight, float vfact1, float vfact2,
		float pfact,float theta, float pmass){
	long i,j,k;
	long ix0,iy0,iz0;
	long ix2,iy2,iz2;
	long jxp,jyp,jzp;
	...
	*/
	...
	printf("P%d has np=%ld mx=%d my=%d mz=%d memory for basicell = %ld zstart=%g vfact2=%g\n",
			myid,np, mx,my,mz, sizeof(treeparticletype *)*mx*my*mz,zstart,vfact2);
	vfact2 = vfact2* pmass;
	p = particles;
	printf("P%d x0=%g y0=%g z0=%g\n",myid,p->x,p->y,p->z);
	...
	ParticleCellLinkedList(particles,np,zstart);
	{
...
#pragma omp parallel
	...
	for(iz0=0;iz0<mz;iz0+=nstride){
		 ...
		 * ("my" / "nthreads") % "nstride" should be 0 */
#pragma omp parallel private(iy0,jyp,jxp,ix0,ngrv,npos,i,box)
		{
			int siy0,ncell;
			...
			for(iy0=0;iy0<my;iy0+=nstride){
				...
				for(ix0=0;ix0<mx;ix0+=nstride){
					...
					if(ngrv > 0){
						npos = Dump2PeriodicPosExceptZ(ix0,iy0,iz0,-1,jxp,-1,jyp,-1,jzp,nx,ny,threadpos);
						/*
						TIMER_START(23);
						*/
#ifndef USE_GPU
						if(ngrv >= DIRECTSUM){
							box.x = ix0*CellWidth;
							box.y = iy0*CellWidth;
							box.z = iz0*CellWidth+zstart;
							box.width = nstride*CellWidth;
		
							Make_Tree(threadTREECELL,threadgrv,ngrv,box,&ncell);
							for(i=0;i<npos;i++)
						...
						/*
						TIMER_STOP(23);
						if(myid==0 && iz0>1 && iz0<mz-1) printf("TREEWALKTIMER %d %d %g\n",ngrv, npos,ELAPSED_TIME(23)/(float)npos);
						fflush(stdout);
						*/
						if(0) {
							FILE *wp;
							if(myid==0){
								...
								for(i=0;i<npos;i++){
									fprintf(wp,"%d %g %g %g\n",i,threadpos[i].ax,threadpos[i].ay,threadpos[i].az);
								}
								fclose(wp);
							}
							MPI_Finalize();exit(999);
						}
						PartialUpdateAccelUsingForceByParticles(vfact2,npos,threadpos);
					}
				}
			}
		}
...
#pragma omp parallel
	...
	/*
    if(myid==0) fprintf(stdout,"Main Tree. CPU= %f \n", ELAPSED_TIME(1));
	TIMER_START(1);
	*/
	{
		MPI_Barrier(MPI_COMM_WORLD);
		if(myid==0) printf("P%d now entering ghost routine 1\n",myid);
		Ghost(1,zstart,zheight,vfact2,nx,ny,nz,pmass);
		MPI_Barrier(MPI_COMM_WORLD);
		if(myid==0) printf("P%d now entering ghost routine -1\n",myid);
		Ghost(-1,zstart,zheight,vfact2,nx,ny,nz,pmass);
		MPI_Barrier(MPI_COMM_WORLD);
...
void Ghost(long iflag,float zstart,float zheight,float vfact2,
		int nx, int ny, int nz,float pmass){
	long i,j,k;
	...
	{ /* Check whether communication beyond contact neighbors is indispensable.
	...
	for(iy0=0;iy0<my;iy0+=nstride){
		...
		if(tnneighbor > 1){
			for(i=2;i<=tnneighbor;i++){
				...
				printf("P%d put to P%d np=%d & get from P%d np=%d\n"
		...
		/*
				if(iflag ==1){
					for(j=0;j<nrghost;j++){
						if(rghost[j].z < zstart-rsphere){
							printf("Error in P%d for %d in %d : %g : %g, iy0=%d\n",myid,j,nrghost,
									zstart,rghost[j].z,iy0);
							break;
						}
					}
				}
...
#pragma omp parallel private(ix0,ix1,jxp,ngrv,npos,box,i,ix)
		{
			int idthread,nthreads,ncell;
				...
				if(ngrv > 0){
					...
					for(ix=ix0;ix<ix1;ix++){
						...
						while(tp){
							...
							threadgrv[ngrv].r[0] = tp->x;
							threadgrv[ngrv].r[1] = tp->y;
							threadgrv[ngrv].r[2] = tp->z;
							threadgrv[ngrv].mass = 1;
							tp = tp->next;
							ngrv ++;
						}
					}
					npos = Dump2PeriodicPosExceptZ(ix0,iy0,iz0pos,-1,jxp,-1,jyp,jzm,jzp,nx,ny,threadpos);
					if(npos > 0){
#ifndef USE_GPU
						if(ngrv >= DIRECTSUM){
