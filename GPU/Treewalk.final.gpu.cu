...
__global__ void kernel_PtclGravityMeasure(particle *dp,int npos,
	...
	int idx = (blockDim.x*blockIdx.y+blockIdx.x)*threadSize+ threadIdx.x; 
	if(idx< npos){
		float dist2;
...
#define PARTICLEFORCE(p,ptr,rspheresq){\
	...
	{\
		...
		accel[idx].x += tmpx*fplmf;\
		accel[idx].y += tmpy*fplmf;\
		accel[idx].z += tmpz*fplmf;\
	}\
}
#define CELLFORCE(p,ptr,rspheresq){\
	...
	{\
		...
		float twofplmf2 = 2.*fplmf.y;\
		accel[idx].x += tmpx*tmptmp+qxx1*twofplmf2;\
		accel[idx].y += tmpy*tmptmp+qxx2*twofplmf2;\
		accel[idx].z += tmpz*tmptmp+qxx3*twofplmf2;\
	}\
}
__global__ void All_gpu_treeforce(particle *pp, int npos, TStruct *tree,TPtlStruct *grv,
		float rspheresq){
	__shared__ float4 accel[threadSize];
	void *ptr;
	float tmpx,tmpy,tmpz; 
	float dist2; 
	TStruct *cell;
	...
	accel[idx] = make_float4(0.f,0.f,0.f,0.f);
	if(idx<npos){
		particle *p = &(pp[idx]);
		ptr = (void*)tree;
		while(ptr != NULL){
		...
		p->ax = accel[idx].x; p->ay = accel[idx].y; p->az = accel[idx].z;
	}
}
...
extern "C" void gputreeforce(particle *pos,int npos,
		...
		float rspheresq, int CpusPerGpu, int myid, int nx){
	...
	if(initialflag){
		...
		CUDA_SAFE_CALL( cudaMallocHost(PPTR(hforce1),sizeof(float)*NTextureSize));
		CUDA_SAFE_CALL( cudaMallocHost(PPTR(hforce2),sizeof(float)*NTextureSize));
		CUDA_SAFE_CALL( cudaMallocHost(PPTR(hforce3),sizeof(float)*NTextureSize));
		...
		CUDA_SAFE_CALL( cudaMemcpy(force3,hforce3,sizeof(float)*NTextureSize,
					cudaMemcpyHostToDevice));
		CUDA_SAFE_CALL( cudaFreeHost(hforce3));
		CUDA_SAFE_CALL( cudaFreeHost(hforce2));
		CUDA_SAFE_CALL( cudaFreeHost(hforce1));
	...
	{/* This is for transferring TREE data to device memory */
		( cudaMalloc(PPTR(dtree),sizeof(TStruct)*ncell));
		( cudaMalloc(PPTR(dpgrv),sizeof(TPtlStruct)*ngrv));

		for(int i=0;i<ncell;i++){/* flagging for sibling and daughter pointers */
		...
		( cudaMemcpy(dtree,TREECELL,sizeof(TStruct)*ncell,cudaMemcpyHostToDevice));
		( cudaMemcpy(dpgrv,grv,sizeof(TPtlStruct)*ngrv,cudaMemcpyHostToDevice));

		/* */
		( cudaMalloc(PPTR(dpos),sizeof(particle)*npos));
		( cudaMemcpy(dpos,pos,sizeof(particle)*npos,cudaMemcpyHostToDevice));


		blockSize = GetBlockSize1D(npos,threadSize);
		All_gpu_treeforce<<<blockSize,threadSize>>>(dpos,npos,dtree,dpgrv,rspheresq);
		( cudaMemcpy(pos,dpos,sizeof(particle)*npos,cudaMemcpyDeviceToHost));
	...
	*/
	...
	stepnpos = mydev.totalGlobalMem/(2.*sizeof(oContactGrav)*NPCMAX)/CpusPerGpu*0.7;
