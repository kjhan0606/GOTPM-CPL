...
static treeparticletype **BasicCell;
static long *NBasicCell;
static int nullfct0() {
return 0;
}
int nullfct1() {
...
#pragma omp master
	...
	int mwidth=(int)(ceil((double)nx*(double)invCellWidth));
	long nwidth = (mwidth+nthreads-1)/nthreads;
	unsigned char *target = (unsigned char*)Malloc(sizeof(unsigned char)*np, PPTR(target));
	{

		long i;
		treeparticletype *tp = particles;
#pragma omp parallel for
		for(i=0;i<np;i++){
			unsigned char ix = (long)(XofP(particles+i)*invCellWidth)/nwidth;
...
#pragma omp parallel
	{
		...
		long i;
		treeparticletype *p, *tp = particles;
		for(i=0;i<np;i++){
			if(target[i] != idthread) {
				tp++;
				continue;
			}
			long ix,iy,iz;
			ix = (long)(XofP(particles+i)*invCellWidth);
			iy = (long)(YofP(particles+i)*invCellWidth);
			iz = (long)((ZofP(particles+i)-zstart)*invCellWidth);
			ix = min(ix,mxm1);
			iy = min(iy,mym1);
			iz = min(iz,mzm1);
			if(ix < 0) ix = 0;
...
void BoundaryCellLinkedList(ComGhost *bparticles, long nbp, 
		ComGhost **BoundaryCell, long *NBoundaryCell, int mbx){
	long i;
	for(i=0;i<mbx;i++){
		BoundaryCell[i] = NULL;
		NBoundaryCell[i] = 0;
...
#pragma omp master
	...
	int mwidth = (int)(ceil((double)simpar.nx*(double)invCellWidth));
	long nwidth = (mwidth+nthreads-1)/nthreads;
	unsigned char *target = (unsigned char*)Malloc(sizeof(unsigned char)*nbp, PPTR(target));
	{
		long i;
#pragma omp parallel for
		for(i=0;i<nbp;i++){
			unsigned char ix = (long)(bparticles[i].x/CellWidth);
			if(ix>=nthreads) ix = nthreads -1;
			target[i] = ix;
		}
	}
#pragma omp parallel
	{
		int idthread = omp_get_thread_num();
		long i;
		ComGhost *p,*tp = bparticles;
		for(i=0;i<nbp;i++){
			if(target[i] != idthread){
				tp++;
				continue;
			}
			long ix = (long)(tp->x/CellWidth);
			ComGhost *p = BoundaryCell[ix];
			BoundaryCell[ix] = tp;
			tp->next = p;
			NBoundaryCell[ix] ++;
			tp++;
...
//				pos[i].ax += tmpx* fplmf * pcorr;
//				pos[i].ay += tmpy* fplmf * pcorr;
//				pos[i].az += tmpz* fplmf * pcorr;
				pos[i].ax += tmpx* fplmf;
				pos[i].ay += tmpy* fplmf;
				pos[i].az += tmpz* fplmf;
...
//		bp->vz += vfact2*pos[i].az;		
		float pcorr = simpar.pcorr;
		bp->vx += vfact2*pos[i].ax*mass*pcorr;
		bp->vy += vfact2*pos[i].ay*mass*pcorr;
		bp->vz += vfact2*pos[i].az*mass*pcorr;
// 2024_06_21 minseong velocity pcorr restored
	}
}
/*
...
#pragma omp parallel
#endif
		{
			if(omp_get_thread_num()==0) {
				pos = (particle *)Malloc((size_t)sizeof(particle)*(size_t)mpos*(size_t)omp_get_num_threads(),PPTR(pos));
				grv = (TPtlStruct *) Malloc((size_t)sizeof(TPtlStruct)*(size_t)mgrv*(size_t)omp_get_num_threads(),PPTR(grv));
				TREECELL = (TStruct *)Malloc((size_t)sizeof(TStruct)*(size_t)mgrv*(size_t)omp_get_num_threads(),PPTR(TREECELL));
			}
		}
	}
#ifdef USE_GPU
