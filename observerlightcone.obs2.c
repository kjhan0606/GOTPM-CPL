...
void Obs2ExtractingLightConeParticles(pmparticletype *pmparticles,int np,float nextastep,float maxd){
	int nx,ny,nz, nspace;
	...
	*/
	...
	OBSTEMWRITE(slcparticles,snlcp,myid,nid,2);
	/*
	for(i=0;i<snlcp;i++){
...
void Obs2SavingLightConeData(pmparticletype *pmparticles,int np,int nx,int ny,
	...
	OBSTEMREAD(slcparticles,snlcp,myid,nid,2);
	...
	*/
	...
	}
	{
		...
		sprintf(surveyfilename,"Olightcone2.main.%.5d",mstep);
		pp = lightcones;
		writedownlightconedata(surveyfilename,pp,nnp[0]);

		sprintf(surveyfilename,"Olightcone2.outer.%.5d",mstep);
		pp += nnp[0];
		writedownlightconedata(surveyfilename,pp,nnp[1]);

		sprintf(surveyfilename,"Olightcone2.inner.%.5d",mstep);
		pp += nnp[1];
