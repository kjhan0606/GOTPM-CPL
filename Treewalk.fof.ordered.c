...
void WriteFinalHalo(size_t nhalo, HaloBound *halobound,FoFTPtlStruct *ptl,
	...
	for(i=0;i<nid;i++){
		if(i==myid){
			hfp=fopen(halofile,"a");
			pfp=fopen(memparticlefile,"a");
			fprintf(stdout,"P%d is writing data to %s\n",myid,memparticlefile);
			for(j=0;j<nhalo;j++){
				ptr = halobound[j].sibling;
				np = 0;
				while(ptr){
...
void CheckHaloBound(size_t nhalo,HaloBound *halo,FoFTPtlStruct *ptl,size_t np,
	...
	printf("P%d Maximum FoF member of particles %ld\n",myid,maxnmem);
	{
		...
		printf("P%d has num0=%ld num1=%ld num2=%ld num3=%ld ::: %g %g\n",myid,num0,num1,num2,num3,zminlocal,zup);
	}
}
