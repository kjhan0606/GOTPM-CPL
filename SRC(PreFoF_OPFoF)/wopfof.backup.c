...
POSTYPE lx,ly,lz;
	...
	float size,hubble,npower,omep,omepb,omeplam,bias,smooth,wlam;
	int nx,ny,nz,nspace;
...
int main(int argc,char *argv[]){
	...
	if(argc != 3) {
		...
		fscanf(ffp,"%g %g %g %g %g %g",&npower,&omep,&omepb,&omeplam,&bias,&smooth);
		...
		*/
		...
		N3 = N3*0.5;
		rscale = size/nx;
		HSUB = sqrt(omep*pow3(amax/a)+omeplam*pow(amax/a,3*(1+wlam))+(1.-omep-omeplam)*pow2(amax/a));
		vscale = size/amax*a*a*100.*HSUB;
           
		fof_link = fof_link * nspace;
		if(myid==0){
			printf("P%d size = %g hubble = %g wlambda= %g\n",myid,size,hubble, wlam);
	...
	box.x = box.y = box.z = 0.;
	box.width = nx;
	sprintf(halofile,"wFoF_halo_cat.%.5d",nstep);
	sprintf(memparticlefile,"wFoF_member_particle.%.5d",nstep);
	if(myid==0){
		...
		fwrite(&wlam,sizeof(float),1,hfp);
	...
	for(nowfile=initfile;nowfile<finalfile;nowfile++){
		...
		if(np /NODE_HAVE_PARTICLE *2.0 > ntreemax){
			ntreemax = np/NODE_HAVE_PARTICLE *2.0;
