...
enum boolean pfof_open(particle p,FoFTStruct *tree, POSTYPE fof_link, 
		POSTYPE Lx2, POSTYPE Ly2, POSTYPE Lz2,
...
void WriteIsolatedHalo(size_t nhalo, HaloBound *halobound,FoFTPtlStruct *ptl,
...
#ifndef MulOut
	{
		MPI_Status rstatus;
		i = 1;
		MPI_Send(&i,1,MPI_INT,0,WRITE_TAG1,MPI_COMM_WORLD);
		MPI_Recv(&i,1,MPI_INT,0,WRITE_TAG2,MPI_COMM_WORLD, &rstatus);
	}
#endif
	int *ok = (int*)Malloc(sizeof(int)*nhalo, PPTR(ok));
#pragma omp parallel for 
	for(j=0;j<nhalo;j++){ 
		ok[j] = 0;
		if(halobound[j].boundflag ==0){ 
			FoFTPtlStruct *ptr = halobound[j].sibling; 
			size_t np = 0; 
			while(ptr){
				ptr = ptr->sibling;
				np++;
	...
	i = 0;
#ifndef MulOut
	MPI_Send(&i,1,MPI_INT,0,WRITE_TAG3,MPI_COMM_WORLD);
#endif
}
...
void ReadBottomFaceContact(FoFTPtlStruct *p,size_t npread, particle *linked,int src,
	...
	}
	{
			...
			printf("P%d:  Now read %ld bottom particles from %ld particles\n",myid, tmp-p,npread);
	}
	fflush(fp);
	fclose(fp);
}
...
size_t WriteBottomFaceContact(size_t nhalo, HaloBound *halobound,
		FoFTPtlStruct *ptl, particle *linked,int nowfile,int nstep, int myid){
	int i;
	size_t j;
	size_t np;
	HaloQ haloq;
...
size_t StackUpContactParticleLeftWard(size_t nhalo,HaloBound *halobound,
	...
	nowp = 0;
	for(i=0;i<np;i++){
		long long mh = ptl[i].haloindx;
		if(mh>=0 && (halobound[mh].boundflag ==2 || halobound[mh].boundflag ==3)){
			ptl[nowp] = ptl[i];
			nowp ++;
		}
	}
...
size_t pnew_fof_link(particle *p,POSTYPE fof_link,FoFTStruct *tree,
		FoFTPtlStruct *ptl,particle *linked,INDXTYPE *tindx, size_t nhalo,
		POSTYPE lx,POSTYPE ly, POSTYPE lz){
	size_t ncount, now;
	void *ptr,*optr,*nptr;
	POSTYPE tmpx,tmpy,tmpz;
		...
		while(ptr != NULL){
			switch(((TYPE*)ptr)->type){
				case TYPE_TREE:
					...
					switch(pfof_open(point,ptr,fof_link, Lx2, Ly2, Lz2, Lx, Ly, Lz)){
						case YES:
							optr = ptr;
				...
				default :
					if(((FoFTPtlStruct*)ptr)->included == YES){
						...
						if(dist2 <= fof_link){
							...
							/* This is to save the indexing of FoFTPtlStruct, which might be referred to when
							 * the halo criteria are not satisfied. */
							tindx[ncount] = ((FoFTPtlStruct*)ptr)->tindx;
...
#pragma omp master
	...
	FoFTPtlStruct **Grids1D = (FoFTPtlStruct **)Malloc(sizeof(FoFTPtlStruct*)*nthreads,PPTR(Grids1D));
