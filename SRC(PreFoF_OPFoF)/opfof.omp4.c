	...
	*/
	...
	if(amyid==0) {
		...
		do{
			...
			while( (wblock == 1 && rstatus.MPI_TAG == WRITE_TAG1) || 
				(isyncRead >= maxReads && rstatus.MPI_TAG == 2) ) {
				int flag;
				if( (wblock == 1 && rstatus.MPI_TAG == WRITE_TAG1) &&
						(isyncRead >= maxReads && rstatus.MPI_TAG == 2) ){
        			MPI_Iprobe(MPI_ANY_SOURCE, 3,MPI_COMM_WORLD,&flag, &rstatus);
					if(!flag){
        				MPI_Iprobe(MPI_ANY_SOURCE, WRITE_TAG3,MPI_COMM_WORLD,&flag, &rstatus);
					}
				}
				else if( (wblock == 1 && rstatus.MPI_TAG == WRITE_TAG1) ){
					MPI_Iprobe(MPI_ANY_SOURCE, WRITE_TAG3, MPI_COMM_WORLD,&flag, &rstatus);
					if(!flag) {
						MPI_Iprobe(MPI_ANY_SOURCE, 3, MPI_COMM_WORLD,&flag, &rstatus);
						if(!flag){
							MPI_Iprobe(MPI_ANY_SOURCE, 2, MPI_COMM_WORLD,&flag, &rstatus);
						}
					}
				}
				else if( isyncRead >= maxReads && rstatus.MPI_TAG == 2){
					MPI_Iprobe(MPI_ANY_SOURCE, 3, MPI_COMM_WORLD,&flag, &rstatus);
					if(!flag){
						MPI_Iprobe(MPI_ANY_SOURCE, WRITE_TAG3, MPI_COMM_WORLD,&flag, &rstatus);
						if(!flag){
							MPI_Iprobe(MPI_ANY_SOURCE, WRITE_TAG1, MPI_COMM_WORLD,&flag, &rstatus);
						}
					}
				}
			}
			...
			if(tag==3){
				...
				if(wblock){
					printf("Error: request of overlap writing from G%d\n", src);fflush(stdout);
				}
				else {
					printf("P%d -> G%d: a permission to write data\n", myid, src-1);
				}
				fflush(stdout);
			...
			else if(tag == WRITE_TAG3){
            	MPI_Recv(&ok, 1, MPI_INT,src, WRITE_TAG3, MPI_COMM_WORLD,&rstatus);
				printf("P%d <- G%d: a notification of closing the writing of halo data\n", myid, src-1);
				fflush(stdout);
				wblock = 0;
			}
