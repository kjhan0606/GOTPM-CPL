...
#pragma omp parallel private(i)
	{
		...
		long long widthTree = ntreemax/nthreads;
		FoFTStruct *thread_TREE_START = TREE_START + idthread*widthTree;
		thread_TREE_START->sibling = thread_TREE_START + widthTree;
		if(idthread == nthreads-1) thread_TREE_START->sibling = NULL;
		FoFTPtlStruct *ptr;
		FoFBeginEndTree beginend;
		beginend = FoF_divide_node(thread_TREE_START,thread_TREE_START+1,
...
//	ThisTree->dist = sqrt(distmax);
	...
	if(tmpnode[i].Nparticle >= MIN_CELL_PARTICLE_NUM){
