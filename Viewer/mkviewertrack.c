...
int main ( int argc, char **argv){
	...
	for(i=nstep4;i<nstep5;i++){
		vx = 1.;
		xrot += drot*erf((i+1.-nstep4)/nsmooth);
		pz += dz*erf((i+1.-nstep4)/nsmooth);
		printf("%d %d %g %g %g 1 0 1 1 0 -1 %g %g 0\n",i+1,stepcount,px,py,pz,xrot,yrot);
	}
	...
	for(i=nstep5;i<nstep6;i++){
		amp += erf((nstep6-i)/nsmooth);
	}
	drot = PI/2/amp;
	dz = -dist/2/amp;

	for(i=nstep5;i<nstep6;i++){
		xrot += drot*erf((nstep6-i)/nsmooth);
		pz += dz*erf((nstep6-i)/nsmooth);
		printf("%d %d %g %g %g 1 0 1 1 0 -1 %g %g 0\n",i+1,stepcount,px,py,pz,xrot,yrot);
	}
}
