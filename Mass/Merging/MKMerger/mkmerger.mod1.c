...
#define NSTEPS 142
int nsteps[NSTEPS] ={
130,154,176,198,218,237,256,274,291,308,325,341,357,373,388,403,418,433,447,461,475,489,503,
...
void advdumpdata(SimplifiedHalo *shnow, IDTYPE npnow,char *outfile){
	...
	for(j=0;j<nid;j++)
	{
...
#endif
		{
			if(myid==0) {
				wp = fopen(outfile,"w");
				fwrite(&globalid,sizeof(IDTYPE),1,wp);
			}
			...
			if(npnow >0) {
				...
				for(i=0;i<npnow;i+= nblockid){
					...
					for(k=0;k<npnow;k++){
						long long ioffset = shnow[k].globalid - (runninggid);
						if(ioffset <0) continue;
						else if(ioffset >= mp) break;
						else {
...
int main(int argc, char **argv){
	...
	*/
	...
	sprintf(infile1,"HaloRVList.%.5d",nsteps[istart]);
	sprintf(infile2,"FoF_halo_cat.%.5d",nsteps[istart]);
	sprintf(infile3,"HaloRVList_halo.%.5d",nsteps[istart]);
	...
	if(istart ==0){
		...
		char out1[100]; sprintf(out1,"%s.%.5d",outfile, istart-1);
		npnow = readpredumpdata(shnow,npnow,out1);
	}
