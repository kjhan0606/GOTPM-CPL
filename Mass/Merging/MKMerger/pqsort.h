...
void my_MPI_Sendrecv(void *, long, MPI_Datatype , int , int , void *, long , 
...
#define GenerateParallelComp(TYPE,subtype,mem)  \
	...
	int Psort_##TYPE##_##subtype##_##mem##_L(const void *a, const void *lb, const void *lc){\
		if(((TYPE*)a)->mem >= LONG_MAX) return 1;\
		else if(((TYPE*)lb)->mem == ((TYPE*)lc)->mem) return 1;\
...
#define MyFixedRangePqsort(base,nmem,TYPE,subtype,mem,min,max,Comm) do{\
	int Psort_##TYPE##_##subtype##_##mem(const void *, const void *);\
	int Psort_##TYPE##_##subtype##_##mem##_L(const void *, const void *, const void *);\
	int Psort_##TYPE##_##subtype##_##mem##_R(const void *, const void *, const void *, int);\
	int _id,_mid;\
	...
	lmax.mem = max;\
	printf("-P%d has %s local min/max = %ld %ld for %ld members\n",\
			_id,#base"."#mem,(long)lmin.mem,(long)lmax.mem,(long)nmem);fflush(stdout);\
	size_t mmem = nmem;\
	base = (TYPE*)pqsort(base,&mmem,sizeof(TYPE),Psort_##TYPE##_##subtype##_##mem##_R, Psort_##TYPE##_##subtype##_##mem##_L, \
			Psort_##TYPE##_##subtype##_##mem,&lmin,&lmax,Comm);\
	printf("+P%d has %s local min/max = %ld %ld for %ld members\n",\
			_id,#base"."#mem,(long)lmin.mem,(long)lmax.mem,(long)mmem);fflush(stdout);\
	nmem = mmem;\
} while(0)

/*
#define RedMyFixedRangePqsort(base,nmem,TYPE,subtype,mem,ired,min,max,Comm) do{\
	int Psort_##TYPE##_##subtype##_##mem(const void *, const void *);\
	int Psort_##TYPE##_##subtype##_##mem##_L(const void *, const void *, const void *);\
	int Psort_##TYPE##_##subtype##_##mem##_R(const void *, const void *, const void *, int);\
	int _id,_mid;\
	...
	lmax.mem[ired] = max;\
	printf("-P%d has %s local min/max = %ld %ld for %ld members\n",\
			_id,#base"."#mem,(long)lmin.mem##[ired],(long)lmax.mem##[ired],(long)nmem);fflush(stdout);\
	size_t mmem = nmem;\
	base = (TYPE*)pqsort(base,&mmem,sizeof(TYPE),Psort_##TYPE##_##subtype##_##mem##_R, Psort_##TYPE##_##subtype##_##mem##_L, \
			Psort_##TYPE##_##subtype##_##mem,&lmin,&lmax,Comm);\
	printf("+P%d has %s local min/max = %ld %ld for %ld members\n",\
			_id,#base"."#mem,(long)lmin.mem##[ired],(long)lmax.mem##[ired],(long)mmem);fflush(stdout);\
	nmem = mmem;\
} while(0)
#define RedMyFixedRangePqsort0(base,nmem,TYPE,subtype,mem,ired,min,max,Comm) do{\
	int Psort_##TYPE##_##subtype##_##mem##0(const void *, const void *);\
	int Psort_##TYPE##_##subtype##_##mem##_L(const void *, const void *, const void *);\
	int Psort_##TYPE##_##subtype##_##mem##_R(const void *, const void *, const void *, int);\
	int _id,_mid;\
	MPI_Datatype mpi_datatype;\
	...
	size_t mmem = nmem;\
	base = (TYPE*)pqsort(base,&mmem,sizeof(TYPE),Psort_##TYPE##_##subtype##_##mem##_R, Psort_##TYPE##_##subtype##_##mem##_L, \
			Psort_##TYPE##_##subtype##_##mem##0,&lmin,&lmax,Comm);\
	printf("+P%d has %s local min/max = %ld %ld for %ld members\n",\
			_id,#base"."#mem,(long)lmin.mem##[ired],(long)lmax.mem##[ired],(long)mmem);fflush(stdout);\
	nmem = mmem;\
} while(0)
