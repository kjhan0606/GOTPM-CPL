...
*/
        for(i=0;i<Nsample;i++){
		msam[i].minM = pow(10.L, 12.L + i*(log(5e15)-12.L)/Nsample);
		msam[i].maxM = pow(10.L, 12.L + (i+1)*(log(5e15)-12.L)/Nsample);
	}
	...
	for(i=0;i<Nsample;i++){
		for(j=0;j<msteps;j++){
			msam[i].red[j] = reds[j];
			for(k=0;k<1024;k++) msam[i].Mnow[j][k] = 0;
		}
	}
	...
	FILE *wp = fopen("finesample_mergingdist.out","w");
	for(i=0;i<Nsample;i++){
