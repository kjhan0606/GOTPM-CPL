...
/* This is for CPL Run1 simulation */
#define NSTEPS 130
int nsteps[NSTEPS] ={
...
/* This is for CPL Run0 simulation */
/*
#define NSTEPS 133
int nsteps[NSTEPS] ={ 
...
IDTYPE readhidmass(char *infile2,HidMass **hidmass){
	...
	{
		FILE *fp = fopen(infile2,"r");
		int nshift = sizeof(float)*12 + sizeof(int)*2 + nstart*sizeof(Halo);
		fread(&boxsize,sizeof(float),1,fp);
		fread(&hubble,sizeof(float),1,fp);
		fread(&npower,sizeof(float),1,fp);
		fread(&omep,sizeof(float),1,fp);
...
void advdumpdata(SimplifiedHalo *shnow, IDTYPE npnow,char *outfile, int istep){
	IDTYPE i,j,k,nshift,gidsize;
	FILE *wp;
	...
	if(0){
		IDTYPE pgid;
		if(myid!=0){
			MPI_Recv(&pgid, 1,MPI_LONG, myid-1, 0, MPI_COMM_WORLD, &status);
			if(npnow > 0 && pgid >= shnow[0].globalid){
				printf("P%d Error here %ld %ld\n",myid, pgid, shnow[0].globalid);
				exit(999);
			}
		}
		if(myid != nid-1){
			if(npnow >0) pgid = shnow[npnow-1].globalid;
			else pgid = -1;
			MPI_Send(&pgid, 1,MPI_LONG, myid+1, 0, MPI_COMM_WORLD);
		}
		for(i=0;i<npnow-1;i++){
			if(shnow[i].globalid >= shnow[i+1].globalid){
				fprintf(stderr,"Error in sorting globalid %ld : %ld : at %ld\n",shnow[i].globalid, shnow[i+1].globalid, i);
				exit(99);
			}
		}
	}
	...
	for(j=0;j<nid;j++)
	{
...
#endif
		{
			...
			if(npnow >0) {
				...
				*/
				fseek(wp,0 + sgid * gidsize , SEEK_SET);
				for(i=0;i<npnow;i++){
					nshift  = gidsize * (shnow[i].globalid - sgid);
					fseek(wp,nshift,SEEK_CUR);
					fwrite(&(shnow[i].basichalo),sizeof(SavedHaloType),1,wp);
				...
				for(i=sgid;i<=shnow[npnow-1].globalid;i+= nblockid){
					IDTYPE mp = MIN(shnow[npnow-1].globalid+1-runninggid,nblockid);
					SavedHaloType *buff = (SavedHaloType *)calloc(mp, sizeof(SavedHaloType));
					for(k=0;k<mp;k++) buff[k].mbp = -1;
					for(k=0;k<npnow;k++){
						IDTYPE ioffset = shnow[k].globalid - runninggid;
						if(ioffset <0) continue;
						else if(ioffset >= mp) break;
						else {
					...
					fwrite(buff, sizeof(SavedHaloType), mp,wp);
					free(buff);
					runninggid += mp;
				}
#endif
...
void debug(SimplifiedHalo *arr, IDTYPE np, char *infile, int istep){
	return;
	IDTYPE i,j;
	printf("P%d ::: %s\n", myid,infile);
	/*
	...
	*/
	/*
	for(i=0;i<np;i++){
		if(arr[i].basichalo.mbp == 1373610L){
			printf("\n ###################################################\n");
				printf("\n\n\nP%d @step = %d found mbp = %ld at i=%ld with m = %g mgid= %ld with m= %g :: %s\n\n\n",
						myid,istep, arr[i].basichalo.mbp,i,arr[i].basichalo.mass, arr[i].basichalo.majorglobalid,
						arr[i].basichalo.fofhmass, infile);
				printf("\n ###################################################\n");
			}
		}
		*/
...
int main(int argc, char **argv){
	...
	*/
	...
	sprintf(infile2,"cplFoF_halo_cat.%.5d",nsteps[0]);
	...
	initshift = sizeof(float)*10 + sizeof(int)*1 + (sizeof(int)+sizeof(float))*NSTEPS;
...
#else
	{
		...
		advdumpdata(shnow, npnow, out1,0);
	}
#endif
	...
	for(i=1; i<NSTEPS  ;i++){
		...
		*/
		...
		/* assign globalid to the next-step mbp data */
		{
			...
			max = MIN(max,gmax);
			if(myid==nid-1) max = gmax + 1;
			MPI_Allreduce(&npnow,&bmem, 1, MPI_LONG, MPI_SUM, MPI_COMM_WORLD);
			MyFixedRangePqsort(shnow,npnow,SimplifiedHalo,IDTYPE,upaid,min,max,
					MPI_COMM_WORLD);
			MPI_Allreduce(&npnow,&amem, 1, MPI_LONG, MPI_SUM, MPI_COMM_WORLD);
			if(bmem != amem) { fprintf(stderr,"Error in matching %ld : %ld\n", bmem, amem);exit(999);}
			MPI_Allreduce(&npnow,&bmem, 1, MPI_LONG, MPI_SUM, MPI_COMM_WORLD);
		...
		debug(shup,npup,"11111",i);


		/* assign global id which is the array index that will be save to a file */
		if(myid!=0) {
			MPI_Request request;
			MPI_Irecv(&globalid,1,MPI_IDTYPE,myid-1,0,MPI_COMM_WORLD,&request);
			MPI_Wait(&request, &status);
		}
		printf("P%d starts with globalid = %ld",myid,globalid);fflush(stdout);
		for(k=0;k<npup;k++){
...
/*
			if(i == NSTEPS-1) FinalizeLastStep(npup,shup);
*/
		}

		debug(shup,npup,"22222",i);
...
#else
		{
			...
			advdumpdata(shup, npup, out1,i);
		}
#endif
		debug(shup,npup,"FINAL",i);

		npnow = npup;
		free(shnow); shnow = shup;
		MPI_Barrier(MPI_COMM_WORLD);
		printf("P%d is here %d ::: %d\n",myid, i, NSTEPS);
	}
