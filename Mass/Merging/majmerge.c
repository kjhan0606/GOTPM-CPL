...
int main(int argc, char **argv){
	...
	printf("Then, find the major most bound particle (mbp) in the halo\n");
	printf("And, then, trace the mbp with redshift printing the mass attached to the mbp\n");
	printf("###########################################################################\n");
	...
	printf("The most massive halo or mbp is selected: m= %g\n",mass);
	...
	for(i=istart -1; i>=0 ;i--){
		...
		fclose(fp1);
		printf("nstep=%d : z= %g , aid= %d mass= %g x/y/z= %g %g %g vx/y/z= %g %g %g\n",nsteps[i],
				iredshift,nextaid,imbp.mass,
				imbprv.x,imbprv.y,imbprv.z,imbprv.vx,imbprv.vy,imbprv.vz);
		nextaid = imbp.downaid;
