...
float boxsize,hubble,npower,omep,omepb,omeplam, wlam0, wlam1;
float bias;
int nx,ny,nz,nspace;
...
/*
if(nexthid == 3061497){
	printf("ilink= %ld jlink= %ld jmajor = %ld\n",(ilink-i),jlink, jmajor);
	for(kk=i;kk<ilink;kk++){
		printf("p%d has mbp= %ld \n",kk,nextmbplist[kk].mbp);
	}
	for(kk=j;kk<j+jlink;kk++){
		printf("u%d has mbp= %ld mass = %g\n",kk,uplink[kk].mbp,uplink[kk].mass);
	}
}
*/
			for(k=i;k<ilink;k++){
...
*/
			mbprv[nnextmbplist].x = bp[i].x*rscale;
			mbprv[nnextmbplist].y = bp[i].y*rscale;
			mbprv[nnextmbplist].z = bp[i].z*rscale;
...
*/
			mbprv[nnextmbplist].x = bp[ii].x*rscale;
			mbprv[nnextmbplist].y = bp[ii].y*rscale;
			mbprv[nnextmbplist].z = bp[ii].z*rscale;
...
int main(int argc, char **argv){
	...
	sprintf(infile1,"cplFoFHaloQuantities%.5d.bin",nstep1);
	sprintf(infile2,"cplFoFHaloQuantities%.5d.bin",nstep2);

	sprintf(infile3,"cplFoF_halo_cat.%.5d",nstep2);
	sprintf(infile4,"cplFoF_member_particle.%.5d",nstep2);
	sprintf(infile5,"cplFoF_halo_cat.%.5d",nstep1);
	sprintf(infile6,"cplFoF_member_particle.%.5d",nstep1);
	...
	*/
	...
	double Hsub = hofez_(&amax, &anow, &omep, &omeplam, &wlam0, &wlam1);
...
/* Create work memory space */
	...
	*/
	...
	if(myid ==motherrank) { 
			...
			if(nnextmbplist > 1000000) {	
				do {
					...
					if(ready == READY){
						...
						my_MPI_Send(nextmbplist,nnextmbplist*sizeof(TrHalo),MPI_BYTE,dest,R_TAG,MPI_COMM_WORLD);
						my_MPI_Send(mbprv,nnextmbplist*sizeof(MbpRV),MPI_BYTE,dest,R_TAG,MPI_COMM_WORLD);
						my_MPI_Send(fofhalorv,nnextmbplist*sizeof(MbpRV),MPI_BYTE,dest,R_TAG,MPI_COMM_WORLD);
						MPI_Send(&nuplink,sizeof(IDTYPE),MPI_BYTE,dest,R_TAG,MPI_COMM_WORLD);
						my_MPI_Send(uplink,nuplink*sizeof(UpLink),MPI_BYTE,dest,R_TAG,MPI_COMM_WORLD);
		...
		while(inextmbplist != finish){
			...
			my_MPI_Recv(nextmbplist+nnextmbplist,inextmbplist*sizeof(TrHalo),MPI_BYTE,motherrank,R_TAG,MPI_COMM_WORLD,&cstatus);
			my_MPI_Recv(mbprv+nnextmbplist,inextmbplist*sizeof(MbpRV),MPI_BYTE,motherrank,R_TAG,MPI_COMM_WORLD,&cstatus);
			my_MPI_Recv(fofhalorv+nnextmbplist,inextmbplist*sizeof(MbpRV),MPI_BYTE,motherrank,R_TAG,MPI_COMM_WORLD,&cstatus);
			...
			my_MPI_Recv(uplink+nuplink,iuplink*sizeof(TrHalo),MPI_BYTE,motherrank,R_TAG,MPI_COMM_WORLD,&cstatus);
	...
	*/
	...
	{
		IDTYPE tnnowmbplist,tnuplink,tnnextmbplist;
		MPI_Reduce(&nnowmbplist,&tnnowmbplist, 1, MPI_IDTYPE,MPI_SUM,0,MPI_COMM_WORLD);
		MPI_Bcast(&tnnowmbplist,1,MPI_IDTYPE,0,MPI_COMM_WORLD);
		MPI_Reduce(&nuplink,&tnuplink, 1, MPI_IDTYPE,MPI_SUM,0,MPI_COMM_WORLD);
		MPI_Bcast(&tnuplink,1,MPI_IDTYPE,0,MPI_COMM_WORLD);
		MPI_Reduce(&nnextmbplist,&tnnextmbplist, 1, MPI_IDTYPE,MPI_SUM,0,MPI_COMM_WORLD);
		MPI_Bcast(&tnnextmbplist,1,MPI_IDTYPE,0,MPI_COMM_WORLD);

		if(myid==0){
			fprintf(stderr,"P%d have missing data %ld : %ld : %ld :: %g missing link\n",
					myid,tnnowmbplist,tnuplink, tnnextmbplist, (tnnowmbplist-tnuplink)/(float)tnnowmbplist*100.);
		}
	}
	...
	if(1){
		...
		if(sizeof(IDTYPE) != sizeof(long )) {
			fprintf(stderr,"Error in matching the size of IDTYPE and long \n");
			MPI_Finalize();exit(999);
		}
		long lmaxhid,ltmaxhid;
		lmaxhid = maxhid;
		MPI_Reduce(&lmaxhid,&ltmaxhid, 1, MPI_LONG, MPI_MAX,0, MPI_COMM_WORLD);
		maxhid = ltmaxhid;
		MPI_Bcast(&maxhid,1, MPI_LONG, 0, MPI_COMM_WORLD);
		max = maxhid+1;

		long lminhid,ltminhid;
		lminhid = minhid;
		MPI_Reduce(&lminhid,&ltminhid, 1, MPI_LONG, MPI_MIN,0, MPI_COMM_WORLD);
		minhid = ltminhid;
		MPI_Bcast(&minhid,1, MPI_LONG, 0, MPI_COMM_WORLD);
		min = minhid;
