...
void halo_properties(int nend,PtlPos *r,PtlVel *vr){
	...
	para_spin = sqrt(sqr(ang_mom[0])+sqr(ang_mom[1])+sqr(ang_mom[2])) *
		sqrt(-tenergy)/G/pntmass*com2real*
	...
	rotation = sqrt(sqr(ang_mom[0])+sqr(ang_mom[1])+sqr(ang_mom[2])) *
		com2real/(1.E3L*pc)*r2kineticfact*pntmass/onesolarmass;
	tmass = pntmass*np;
	{
...
int main(int argc, char **argv){
	...
	if(argc != 3) {
		fprintf(stderr,"Error input : mergertree.exe stepnum density\n");
		exit(99);
	}
   		...
   		while(fread(&mpeak,sizeof(int),1,fp) == 1){
			...
			for(i=0;i<mpeak;i++){
				...
				*/
				...
				if(fread(indx,sizeof(int),nend,fp) != nend){
				...
				cy = cy / (float) nend; 
				cz = cz / (float) nend;
				cx = cx - ((int)cx/(int)rng)*(int)rng;
				cy = cy - ((int)cy/(int)rng)*(int)rng;
				cz = cz - ((int)cz/(int)rng)*(int)rng;
				{
					...
					nfx = nfy = nfz = -999999;
					nbuff = 2*h+1;
					for(iiii = 0;iiii<nend;iiii++){
						nx = MIN(nx,(int) r[iiii].x);
						ny = MIN(ny,(int) r[iiii].y);
						nz = MIN(nz,(int) r[iiii].z);
						nfx = MAX(nfx,(int) r[iiii].x);
						nfy = MAX(nfy,(int) r[iiii].y);
						nfz = MAX(nfz,(int) r[iiii].z);
					}
					nxwidth = nfx-nx;
					nywidth = nfy-ny;
					nzwidth = nfz-nz;
					nxwidth = nxwidth+2*nbuff; 
					nywidth = nywidth+2*nbuff; 
					nzwidth = nzwidth+2*nbuff;
					nx -= nbuff;
					ny -= nbuff;
					nz -= nbuff;
					for(iiii = 0;iiii<nend;iiii++){
					...
					for(j=0;j<nxwidth*nywidth*nzwidth;j++){
						den[j] = 0;
					}
					findden(nend,r,nxwidth,nywidth,nzwidth);
					fmax = 0.;
					for(j=0;j<nxwidth*nywidth*nzwidth;j++){
						fmax = MAX(fmax,den[j]);
					}
					if(fmax >= peak ){
						printf("halo %d has %d particles in %d peaks:  %g\n",
								i,nend,mpeak,fmax);
						{
						...
						fprintf(wp,"%g %f %f %f %d %d %d %g %g %g %g %g %g ",
								...
								mpeak,peakindex,para_spin,pvr,rotation,radius,q,s);
