...
void halo_properties_kw(int nend,PtlPos *r,PtlVel *vr,double *KW){
...
int main(int argc, char **argv){
	...
	*/
	nnowhaloindex = 0;
		sprintf(wfile,"FoFHaloQuantities%.5d.dat",inid);
		sprintf(infile,"FoF_member_particle.%.5d",inid);
		sprintf(headerfile,"FoF_halo_cat.%.5d",inid);
		...
		printf("size=%g rng=%g\n",size,rng);
   		...
   		while(fread(&halo,sizeof(Halo),1,hid) == 1){
			...
			for(i=0;i<mpeak;i++){
			*/
			if(ncount%10000 ==0) printf("now %ld is being processed\n",ncount);
			{
				i = 0;
				nend = halo.np;
				...
				*/
				...
				halo_properties_kw(nend,r,vr,&kw);
				{
					float *px,*py,*pz;
				...
				/*
				printf("%g %f %f %f %f %f %f\n",mass,cx*size/rng,cy*size/rng,cz*size/rng,
						cvx*vscale,cvy*vscale,cvz*vscale);
						*/
				fprintf(wp,"%g %f %f %f %d %d %d %g %g %g %g %g %g ",
						mass,cx*size/rng,cy*size/rng,cz*size/rng,
						ncount,1,1,
						para_spin,sigv,rotation,radius,q,s);
