/* This measures the position and velocity of halos
 * using the most bound member particle.
 * by Juhan Kim on 07/Oct./2010
 * */
#include<stdio.h>
#include<stdlib.h>
#include<stddef.h>
...
indxtype  *indx;
float d[3],v[3][3];
double cx,cy,cz,bx,by,bz;
double cvx,cvy,cvz,pbx,pby,pbz;
double ang_mom[3];
...
void GethaloInfo(vector3d *r,vector3d *vr,int np, HaloInfo *halo){
...
#endif
	{
		...
		halo->pbx = pbx*r2kineticfact/1.E5;
		halo->pby = pby*r2kineticfact/1.E5;
		halo->pbz = pbz*r2kineticfact/1.E5;
		halo->radius = radius;
		halo->sigv = sigv;
		halo->rotL = rotation;
...
int MAIN_(int argc, char *argv[]){
	...
	*/
	...
	if(myid == motherrank) {
		char outfile[190];
		sprintf(outfile,"b%.5dhalomass.dat",nstep);
		wp=fopen(outfile,"w");
	...
	if(myid == motherrank) {
		...
		for(i=0;i<nhalo;i++){
			fprintf(wp,"%g %f %f %f %d %d %d %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g\n",
					tt->mass,tt->bx,tt->by,tt->bz,tt->indx,tt->mpeak,tt->isubhalo,tt->spin,tt->sigv,
					tt->rotL,tt->radius,tt->q,tt->s,tt->v[0][0],tt->v[0][1],tt->v[0][2],
					tt->v[1][0],tt->v[1][1],tt->v[1][2],tt->v[2][0],tt->v[2][1],tt->v[2][2],
					tt->ang[0],tt->ang[1],tt->ang[2],tt->pbx,tt->pby,tt->pbz, tt->Es);
			tt++;
