...
int sorthalo(const void *a, const void *b){
	HaloInfo *aa;
	HaloInfo *bb;
	aa = (HaloInfo*)a;
	bb = (HaloInfo*)b;
...
#include "force_spline.h"
*/
void halo_potent(int nend,vector3dD *r,float *penergy){
    TStruct *TREE;
    TPtlStruct *ptl;
    particle p;
    Box box;
...
void halo_properties(int nend,vector3dD *r,vector3d *vr){
...
#include "params.h"
SimParameters param;
void getparameter(FILE *paraid){
	SimParameters read_head(FILE *);
	param = read_head(paraid);
	size = param.boxsize;
	hubble = param.hubble;
	omep = param.omep;
	omepb = param.omepb;
	omeplam = param.omeplam;
	nx = param.nx;
	ny = param.ny;
	nz = param.nz;
	nspace = param.nspace;
	...
	rng = nx;
	amax = param.amax;
	redshift = param.amax/param.anow - 1.;
}
...
void getEs(int np, vector3dD *r, vector3d *vr, HaloInfo *halo){
	int i,j,k;
	int i80,i90;
...
float find_r(vector3dD*,int,float,float,float*,float*,float*,double,double,double);
void GethaloInfo(vector3dD *r,vector3d *vr,int np, HaloInfo *halo){
	int i,j,k;
	float rfact,vfact;
	rfact = rng/size;
...
int main(int argc, char *argv[]) {
	...
	if(argc != 3){
		if(myid == motherrank)
		fprintf(stderr,".... mpimf.exe [nstep] [iflag]\n");
		exit(1);
	}
	nstep = atoi(argv[1]);
	iflag = atoi(argv[2]);

	if(myid==motherrank) nmeg = 10000L;
	else nmeg = 2000L;

	if(Make_Total_Memory(nmeg)==0){
	...
	*/
	if(myid==0){ 
		...
		printf("P%d has omeplam=%g boxsize=%g redshift=%g rscale=%g\n",myid,omeplam,size,redshift,rscale);
	}
	i_potent_spline();

	sprintf(rfilename,"HFIND.DATA.%.5d.%.2d",nstep,iflag);
	if(myid==motherrank) {
		fprintf(stderr,"Opening %s file for input\n",rfilename);
	}
	shalonum = rhalonum = 0;
	if(myid == motherrank) {
		char outfile[190],outbin[190];
		sprintf(outfile,"n%.5dhalomass.%.2d.dat",nstep,iflag);
		sprintf(outbin,"haloinfo.%.5d.%.2d.bin",nstep,iflag);
		wp=fopen(outfile,"w");
		wp2=fopen(outbin,"w");

		index = 0;
		fp = fopen(rfilename,"r");
		printf("P%d is now openning %s \n",myid,outfile);fflush(stdout);
		while(fread(&mpeak,sizeof(int),1,fp) == 1){
			for(i=0;i<mpeak;i++){
				fread(&snp,sizeof(int),1,fp);
				r = (vector3dD *) Malloc(sizeof(vector3dD)*snp,PPTR(r));
				vr = (vector3d *) Malloc(sizeof(vector3d)*snp,PPTR(vr));
				indx = (indxtype*)Malloc(sizeof(indxtype)*snp,PPTR(indx));
				fread(r,sizeof(vector3dD),snp,fp);
				...
				do {
					...
					if(ready == READY){
						...
						MPI_Send(r,snp*sizeof(vector3dD),MPI_BYTE,dest,R_TAG,MPI_COMM_WORLD);
			...
			if(index%1000000 == 0) printf("Now calculating %d halo\n",index);fflush(stdout);
			index ++;
		}
		fclose(fp);
		printf("complete................... \n");
	...
	/* 다른 프로세서는 여기서 계산을 한다  */
		...
		while(snp != finish){
			long numstack;
			snend = MAX(snend,snp);
			r = Malloc(sizeof(vector3dD)*snend,PPTR(r));
			vr = Malloc(sizeof(vector3d)*snend,PPTR(vr));
			indx = Malloc(sizeof(indx)*snend,PPTR(indx));
			MPI_Recv(r,sizeof(vector3dD)*snp,MPI_BYTE,motherrank,R_TAG,MPI_COMM_WORLD,&cstatus);
	...
	if(myid == motherrank) {
		...
		tt = ahalo;
		fwrite(ahalo,sizeof(HaloInfo),nhalo,wp2);
		for(i=0;i<nhalo;i++){
#ifdef XYZDBL
			fprintf(wp,"%g %lg %lg %lg %d %d %d %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g\n",
...
#else
			fprintf(wp,"%g %f %f %f %d %d %d %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g\n",
					tt->mass,tt->cx,tt->cy,tt->cz,tt->indx,tt->mpeak,tt->isubhalo,tt->spin,tt->sigv,
					tt->rotL,tt->radius,tt->q,tt->s,tt->v[0][0],tt->v[0][1],tt->v[0][2],
					tt->v[1][0],tt->v[1][1],tt->v[1][2],tt->v[2][0],tt->v[2][1],tt->v[2][2],
					tt->ang[0],tt->ang[1],tt->ang[2],tt->pvx,tt->pvy,tt->pvz, tt->Es);
