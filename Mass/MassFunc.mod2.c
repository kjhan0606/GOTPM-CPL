...
int main(int argc, char **argv){
    ...
    (void) Make_Total_Memory(3000L);
    i_potent_spline();
    den = (float *) Malloc(256*256*256*sizeof(int),PPTR(den));
    inid = atoi(argv[1]);
    iflag = atoi(argv[2]);
    nnowhaloindex = 0;
        sprintf(wfile,"n%.5dhalomass.%.2d.dat",inid,iflag);
        sprintf(mergerfile,"n%.5dmerger.%.2d.dat",inid,iflag);
        sprintf(infile,"HFIND.DATA.%.5d.%.2d",inid,iflag);
                        ...
                        }
                        {
							...
							*/
        					...
        					for(j=0;j<nend;j++){
								...
								tmpx = r2kineticfact*(vr[j].vx -cvx);
								tmpy = r2kineticfact*(vr[j].vy -cvy);
								tmpz = r2kineticfact*(vr[j].vz -cvz);
