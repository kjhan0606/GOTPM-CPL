...
void halo_potent(int nend,vector3d *r,float *penergy){
    ...
    for(i=0;i<np;i++){
        ...
        penergy[i] = treeplumpotential(&p,theta2,TREE,ptl)*potentfact;
...
exithere :
	...
	if(myid == motherrank) {
		...
		*/
		...
		tt = ahalo;
		for(i=0;i<nhalo;i++){
			fprintf(wp,"%g %f %f %f %d %d %d %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g \n",
					...
					tt->pdr,tt->ddr,tt->tdr,tt->kin,tt->pot, tt->Es);
			tt++;
