...
void Obs1ExtractingLightConeParticles(pmparticletype *pmparticles,int np,float nextastep,float maxd){
	int nx,ny,nz, nspace;
	...
	*/
	...
	OBSTEMWRITE(slcparticles,snlcp,myid,nid,1);
	/*
	for(i=0;i<snlcp;i++){
...
void Obs1SavingLightConeData(pmparticletype *pmparticles,int np,int nx,int ny,
	...
	OBSTEMREAD(slcparticles,snlcp,myid,nid,1);
	...
	*/
	...
	}
	{
		...
		sprintf(surveyfilename,"Olightcone1.main.%.5d",mstep);
		pp = lightcones;
		writedownlightconedata(surveyfilename,pp,nnp[0]);

		sprintf(surveyfilename,"Olightcone1.outer.%.5d",mstep);
		pp += nnp[0];
		writedownlightconedata(surveyfilename,pp,nnp[1]);

		sprintf(surveyfilename,"Olightcone1.inner.%.5d",mstep);
		pp += nnp[1];
