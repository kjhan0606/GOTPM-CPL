...
void PBeamExtractingLightConeParticles(pmparticletype *pmparticles,int np,float nextastep,float maxd){
	int nx,ny,nz, nspace;
	...
	double yrot,phimin,zwidth,mzwidth,xppp,yppp,zppp,zrot,thetamax,thetamin;
	int nbox,ibox;
	int i,j,k;
	long ix,iy,iz,nxy;
	...
	*/
	...
	phimin = 5.L*deg2rad;
	thetamax = 95.L*deg2rad;
	thetamin = 85.L*deg2rad;
	/*
	nbox = 0;
	for(k=-1;k<=0;k++){
		box[ibox].nx = 0.;
...
/*
	zwidth = ZWIDTH*boxsize/(float)nz;
	mzwidth = -1.*zwidth;
*/
	...
	OBSTEMWRITE(slcparticles,snlcp,myid,nid,9);
	/*
	for(i=0;i<snlcp;i++){
...
void PBeamSavingLightConeData(pmparticletype *pmparticles,int np,
	...
	OBSTEMREAD(slcparticles,snlcp,myid,nid,9);
	...
	*/
	...
	}
	{
		...
		sprintf(surveyfilename,"Pencillightcone.main.%.5d",mstep);
		pp = lightcones;
		writedownlightconedata(surveyfilename,pp,nnp[0]);

		sprintf(surveyfilename,"Pencillightcone.outer.%.5d",mstep);
		pp += nnp[0];
		writedownlightconedata(surveyfilename,pp,nnp[1]);

		sprintf(surveyfilename,"Pencillightcone.inner.%.5d",mstep);
		pp += nnp[1];
